...
```typescript
import type { ExtensionAPI } from "@dum-e/coding-agent";
...
| Package | Purpose |
|---------|---------|
| `@dum-e/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@dum-e/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@dum-e/tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@dum-e/coding-agent";
...
```typescript
import { isBashToolResult } from "@dum-e/coding-agent";
...
```typescript
import { createLocalBashOperations } from "@dum-e/coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@dum-e/coding-agent";
...
```typescript
import { SessionManager } from "@dum-e/coding-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@dum-e/ai";
...
```typescript
import type { AutocompleteItem } from "@dum-e/tui";
...
```typescript
import { Box, Text } from "@dum-e/tui";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@dum-e/ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@dum-e/ai";
...
```typescript
import { withFileMutationQueue } from "@dum-e/coding-agent";
...
**Important:** Use `StringEnum` from `@dum-e/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@dum-e/coding-agent";
...
```typescript
import { createBashTool } from "@dum-e/coding-agent";
...
```typescript
import { keyHint } from "@dum-e/coding-agent";
...
```typescript
import { Text, Component } from "@dum-e/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@dum-e/coding-agent";
import { matchesKey } from "@dum-e/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@dum-e/coding-agent";

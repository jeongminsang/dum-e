...
export async function main(args: string[], options?: MainOptions) {
	...
	if (args[0] === "dume" || args[0] === "harness") {
		const subAction = args[1] ?? "status";
		const { HarnessStore } = await import("./dume-harness/store.ts");
		const store = new HarnessStore(join(getAgentDir(), "harness.db"));
		try {
			if (subAction === "doctor") {
			...
			if (subAction === "status") {
				...
				if (args.includes("--json") || args.includes("-j")) {
					...
					console.log(`  Active Unfinished Attempts: ${unfinished.length}`);
				}
				return;
			}
			if (subAction === "goal") {
				...
				const title = titleIdx !== -1 && args[titleIdx + 1] ? args[titleIdx + 1] : "Autonomous DUM-E Mission";
				const requirements = reqIdx !== -1 && args[reqIdx + 1] ? args[reqIdx + 1] : "Execute multi-agent workflow";
				const goal = store.insertGoal({
					id: `goal_${Date.now()}`,
					title,
					requirements,
					requirementsRevision: 1,
					acceptanceCriteria: ["Tests pass", "Verification accepted"],
					status: "active",
					userInterrupted: false,
				});
				if (args.includes("--json") || args.includes("-j")) {
					console.log(JSON.stringify({ ok: true, goal }, null, 2));
				} else {
					console.log(chalk.green(`Created DUM-E Goal ${goal.id}: ${goal.title}`));
				}
				return;
			}
			console.error(`Unknown dume action: ${subAction}. Valid actions: doctor, status, goal`);
			process.exitCode = 1;
			return;
		} finally {
			store.close();

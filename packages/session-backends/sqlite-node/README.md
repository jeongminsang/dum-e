# @dum-e/session-backend-sqlite-node

Node `node:sqlite` Session backend for `@dum-e/agent-core`.

```ts
import { BACKGROUND_CONTEXT } from "@dum-e/agent-core";
import {
  createNodeSqliteFactory,
  SqliteSessionRepo,
} from "@dum-e/session-backend-sqlite-node";

...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@dum-e/evals:session" && artifact.type !== "@dum-e/evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@dum-e/evals:session" ? "sessions" : "sources";

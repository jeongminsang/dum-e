...
Stateful agent with tool execution and event streaming. Built on `@dum-e/ai`.

## Installation

```bash
npm install @dum-e/agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@dum-e/session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.
...
```typescript
declare module "@dum-e/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@dum-e/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@dum-e/agent-core";

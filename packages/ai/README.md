...
```bash
npm install @dum-e/ai
```

TypeBox exports are re-exported from `@dum-e/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@dum-e/ai';
import { builtinModels } from '@dum-e/ai/providers/all';
...
import { openaiProvider } from '@dum-e/ai/providers/openai';
import { openrouterProvider } from '@dum-e/ai/providers/openrouter';
import { amazonBedrockProvider } from '@dum-e/ai/providers/amazon-bedrock';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@dum-e/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@dum-e/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@dum-e/ai';
...
```typescript
import { validateToolCall, type Tool } from '@dum-e/ai';
...
```typescript
import { builtinImagesModels } from '@dum-e/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@dum-e/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@dum-e/ai';
import { openAICompletionsApi } from '@dum-e/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@dum-e/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@dum-e/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@dum-e/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@dum-e/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@dum-e/ai';
import { anthropicProvider } from '@dum-e/ai/providers/anthropic';
import { openaiProvider } from '@dum-e/ai/providers/openai';
import { googleProvider } from '@dum-e/ai/providers/google';
...
- `@dum-e/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@dum-e/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@dum-e/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@dum-e/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@dum-e/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@dum-e/ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @dum-e/ai login              # interactive provider selection
npx @dum-e/ai login anthropic    # login to specific provider
npx @dum-e/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@dum-e/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@dum-e/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@dum-e/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@dum-e/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@dum-e/ai`.

...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	...
	try {
		...
		await writeManifest(root, "packages/evals", {
			...
			private: true,
			dependencies: {
				"@dum-e/coding-agent": "^1.0.0",
				"@mariozechner/pi-ai": "npm:@dum-e/ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@dum-e/coding-agent"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@mariozechner/pi-ai"], "npm:@dum-e/ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@dum-e/coding-agent"], "^1.0.0");
